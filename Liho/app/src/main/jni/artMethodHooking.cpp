...
int (*orig_doCall[4])(void* arg1, void* arg2, void* arg3, void* arg4, int arg5, void* arg6) = {nullptr, nullptr, nullptr, nullptr};
...
int hooked_doCall_indexed(int index, void* arg1, void* arg2, void* arg3, void* arg4, int arg5, void* arg6) {
    std::string pretty = PrettyMethod(arg1, 0);
    //LOGI("hooked doCall[%d]: %s", index, pretty.c_str());
    
    if (pretty.find(APK_NAME) != std::string::npos) {
        LOGI("%s method called: %s", APK_NAME, pretty.c_str());

        for (const auto& hook : get_registered_dex_hooks()) {
            if (pretty == hook.targetMethod) {
                JNIEnv *env = GetEnv();
                if (env == nullptr)
                ...
                jobject newMethodObj = getMethodObject(env, newDexClass, hook.newMethod.c_str(),
                                                       hook.newSig.c_str());

                LOGV("patching original method: %s", pretty.c_str());
